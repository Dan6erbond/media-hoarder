...
const App: Component = () => {
  ...
  const movieStorage = () =>
    showProfiles().length > 0
      ? availableStorage() * 1_000_000 * (ratio() / 100)
      : availableStorage() * 1_000_000;

  const movies = () =>
    movieStorage() / (averageMovieSize() * averageMovieLength);

  const ratios = new Array(21).fill(null).map((_, i) => i * 5);

  let chartElmnt: HTMLCanvasElement | undefined;
  let chart: Chart;
  ...
  onMount(() => {
    ...
    chart = new Chart(chartElmnt!, config);
  });
    ...
    chart.data.datasets = [
      {
        ...
        data: ratios.map((ratio) => {
          ...
          return (
            showStorage /
            (averageEpisodeSize() * averageEpisodes * averageSeasons)
          );
        }),
        tension: 0.1,
      },
      {
        ...
        data: ratios.map((ratio) => {
          ...
          return movieStorage / (averageMovieSize() * averageMovieLength);
        }),
        tension: 0.1,
      },
    ];
  ...
  return (
    <main class="p-8 min-h-screen dark:bg-slate-600 flex gap-6 flex-wrap md:flex-nowrap">
      <div class="flex-1 md:min-w-[450px]">
        ...
        <div class="flex flex-col gap-4 mb-8 md:mb-8 lg:mb-16">
          ...
          <div class="flex flex-col gap-2">
            ...
            <input
              ...
              placeholder="00 TB"
              oninput={(e) =>
                setAvailableStorage(e.currentTarget.valueAsNumber)
              }
        ...
        <div class="flex flex-col items-center justify-center md:flex-row md:items-stretch gap-4 md:gap-8 lg:gap-12">
          {showProfiles().length > 0 && (
            <div class="flex-1 max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 flex flex-col">
